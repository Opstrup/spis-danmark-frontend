 ...
 *
 * @usage
 * To trigger an Action Sheet in your code, use the $ionicActionSheet services in your angular controllers:
 *
 * ```js
 ...
 * @name $ionicBody
 * @module ionic
 * @description An angular utility services to easily and efficiently
 ...
 * @name $ionicGesture
 * @module ionic
 * @description An angular services exposing ionic
 ...
 * browser behaves, an Ionic app is able to keep track of the previous templates, the current templates, and
 * the forward templates (if there is one).  However, a typical web browser only keeps track of one
  ...
  return {
    ...
    register: function(parentScope, viewLocals) {
      ...
      if (forcedNav) {
        ...
        // they went back one, set the old current templates as a forward templates
        ...
        // they went to the forward one, set the forward templates to no longer a forward templates
        ...
        if (forwardView.historyId && tmp.scope) {
          // if a history has already been created by the forward templates then make sure it stays the same
        ...
        // if switching to a different history, and the history of the templates we're switching
        // to has an existing back templates from a different history than itself, then
        // it's back templates would be better represented using the current templates as its back templates
        ...
        if (currentView) {
          // set the forward templates if there is a current templates (ie: if its not the first templates)
          currentView.forwardViewId = viewId;

          action = ACTION_NEW_VIEW;

          // check if there is a new forward templates within the same history
          if (forwardView && currentView.stateId !== forwardView.stateId &&
             currentView.historyId === forwardView.historyId) {
            // they navigated to a new templates but the stack already has a forward templates
            // since its a new templates remove any forwards that existed
          ...
          // there's no current templates, so this must be the initial templates
        ...
        // add the new templates to this history's stack
     ...
     * @description The app's templates history data, such as all the views and histories, along
     * with how they are ordered and linked together within the navigation stack.
     * @returns {object} Returns an object containing the apps templates history data.
     */
     ...
     * @description The app's current templates.
     * @returns {object} Returns the current templates.
     */
     ...
     * @description The ID of the history stack which is the parent container of the current templates.
     ...
     * @description Gets and sets the current templates's title.
     * @param {string=} val The title to update the current templates with.
     * @returns {string} Returns the current templates's title.
     */
     ...
     * @description Returns the templates that was before the current templates in the history stack.
     * If the user navigated from View A to View B, then View A would be the back templates, and
     * View B would be the current templates.
     * @returns {object} Returns the back templates.
     */
     ...
     * @description Gets the back templates's title.
     * @returns {string} Returns the back templates's title.
     */
     ...
     * @description Returns the templates that was in front of the current templates in the history stack.
     * A forward templates would exist if the user navigated from View A to View B, then
     * navigated back to View A. At this point then View B would be the forward templates, and View
     * A would be the current templates.
     * @returns {object} Returns the forward templates.
     */
     ...
     * back. By default it'll go back one templates by using the value `-1`. To go back two
     * views you would use `-2`. If the number goes farther back than the number of views
     * in the current history's stack then it'll go to the first templates in the current history's
     ...
     * @description Navigates the app to the back templates, if a back templates exists.
     */
     ...
     * @description Clears out the app's entire history, except for the current templates.
     */
     ...
     * This both removes the templates element from the DOM, and destroy it's scope.
     */
     ...
     * @description Sets options for the next templates. This method can be useful to override
     * certain templates/transition defaults right before a templates transition happens. For example,
     * the {@link ionic.directive:menuClose} directive uses this method internally to ensure
     * an animated templates transition does not happen when a side menu is open, and also sets
     * the next templates as the root of its history stack. After the transition these options
     * are set back to null.
     *
     * Available options:
     *
     * * `disableAnimate`: Do not animate the next transition.
     * * `disableBack`: The next templates should forget its back templates, and set it to null.
     * * `historyRoot`: The next templates should become the root templates in its history stack.
     *
     * ```js
...
function($rootScope, $state, $location, $document, $ionicPlatform, $ionicHistory, IONIC_BACK_PRIORITY) {
  ...
  $rootScope.$on('$ionicHistory.change', function(e, data) {
    ...
    if (hist && hist.cursor > -1 && hist.cursor < hist.stack.length) {
      ...
      // go to it's last templates in its stack
  ...
  // Set the document title when a new templates is shown
  ...
  function onHardwareBackButton(e) {
    ...
    if (backView) {
      // there is a back templates, go to it
      backView.go();
    } else {
      // there is no back templates, so close the app instead
 ...
 * iOS will move forward by transitioning the entering templates from right to center and the leaving
 * templates from center to left. However, Android will transition with the entering templates going from
 * bottom to center, covering the previous templates, which remains stationary. It should be noted
 ...
 *
 * @param {string} transition Which style of templates transitioning to use.
 *
 ...
 * @description  Maximum number of templates elements to cache in the DOM. When the max number is
 * exceeded, the templates with the longest time period since it was accessed is removed. Views that
 * stay in the DOM cache the templates's scope, current state, and scroll position. The scope is
 * disconnected from the `$watch` cycle when it is cached and reconnected when it enters again.
 * When the maximum cache is `0`, the leaving templates's element will be removed from the DOM after
 * each templates transition, and the next time the same templates is shown, it will have to re-compile,
 ...
 * @returns {number} How many views Ionic will hold onto until the a templates is removed.
 */
 ...
 * navigate forward to the same templates again, it'll create a new DOM element and controllers
 ...
 * @module ionic
 * @description
 * Set the default options to be passed to the {@link ionic.services:$ionicLoading} services.
 *
 * @usage
 * ```js
 ...
 *
 * The Modal is a content pane that can go over the user's main templates
 * temporarily.  Usually used for making a choice or editing an item.
 *
 * Put the content of the modal inside of an `<ion-modal-templates>` element.
 *
 * **Notes:**
...
function($rootScope, $ionicBody, $compile, $timeout, $ionicPlatform, $ionicTemplateLoader, $$q, $log, $ionicClickBlock, $window, IONIC_BACK_PRIORITY) {
   ...
   * @module ionic
   * @description
   * Instantiated by the {@link ionic.services:$ionicModal} services.
   *
  ...
  var ModalView = ionic.views.Modal.inherit({
    ...
    show: function(target) {
      ...
      // if modal was closed while the keyboard was up, reset scroll templates on
...
.service('$ionicNavBarDelegate', ionic.DelegateService([
   ...
   * (if it exists and there is a previous templates that can be navigated to).
  ...
  return {
    $get: ['$q', function($q) {
      var self = {
         ...
         * close the actionsheet, but it should not also go back a page templates
         * or close a modal which may be open.
         *
         * The priorities for the existing back button hooks are as follows:
         *   Return to previous templates = 100
         *   Close side menu = 150
         *   Dismiss modal = 200
         ...
         * previous templates' action, but not any of the other actions.
         *
 ...
 *
 * The Popover is a templates that floats above an app’s content. Popovers provide an
 * easy way to present or gather information from the user and are
 * commonly used in the following situations:
 *
 * - Show more info about the current templates
 * - Select a commonly used tool or configuration
 * - Present a list of actions to perform inside one of your views
 *
 * Put the content of the popover inside of an `<ion-popover-templates>` element.
 *
 * @usage
 * ```html
 * <p>
 ...
 *
 *   // .fromTemplate() method
 *   var template = '<ion-popover-templates><ion-header-bar> <h1 class="title">My Popover Title</h1> </ion-header-bar> <ion-content> Hello! </ion-content></ion-popover-templates>';
 *
...
function($ionicModal, $ionicPosition, $document, $window) {
   ...
   * @module ionic
   * @description
   * Instantiated by the {@link ionic.services:$ionicPopover} services.
   *
 ...
 * @description
 *
 * The Ionic Popup services allows programmatically creating and showing popup
 ...
 *
 * Methods called directly on the $ionicScrollDelegate services will control all scroll
 * views.  Use the {@link ionic.services:$ionicScrollDelegate#$getByHandle $getByHandle}
...
.service('$ionicScrollDelegate', ionic.DelegateService([
   ...
   * @returns {object} The scroll position of this templates, with the following properties:
   ...
   * @description Does not allow this scroll templates to scroll either x or y.
   * @param {boolean=} shouldFreeze Should this scroll templates be prevented from scrolling or not.
   * @returns {boolean} If the scroll templates is being prevented from scrolling or not.
   */
  'freezeScroll',
  /**
   * @ngdoc method
 ...
 *
 * Methods called directly on the $ionicSideMenuDelegate services will control all side
 * menus.  Use the {@link ionic.services:$ionicSideMenuDelegate#$getByHandle $getByHandle}
 ...
 *
 * Methods called directly on the $ionicSlideBoxDelegate services will control all slide boxes.  Use the {@link ionic.services:$ionicSlideBoxDelegate#$getByHandle $getByHandle}
 ...
 *
 * Methods called directly on the $ionicTabsDelegate services will control all ionTabs
 * directives. Use the {@link ionic.services:$ionicTabsDelegate#$getByHandle $getByHandle}
 ...
 * @module ionic
 * @description A services that preemptively caches template files to eliminate transition flicker and boost performance.
 * @usage
...
function($timeout, $document, $q, $ionicClickBlock, $ionicConfig, $ionicNavBarDelegate) {
  ...
  var NAV_VIEW_ATTR = 'nav-templates';
  ...
  var ionicViewSwitcher = {
    ...
    create: function(navViewCtrl, viewLocals, enteringView, leavingView, renderStart, renderEnd) {
      ...
      // loop through to see if the templates is already in the navViewElement
      ...
      var switcher = {
        ...
        loadViewElements: function(registerData) {
          ...
          for (x = 0, l = viewElements.length; x < l; x++) {
            ...
            if (viewEle.data(DATA_ELE_IDENTIFIER) === enteringEleIdentifier) {
              // we found an existing element in the DOM that should be entering the templates
        ...
        render: function(registerData, callback) {
          if (alreadyInDom) {
            ...
            // or the element has cache-templates="false" attribute
            if (viewState(viewLocals).cache === false || viewState(viewLocals).cache === 'false' ||
                enteringEle.attr('cache-templates') == 'false' || $ionicConfig.views.maxCache() === 0) {
          ...
          // update that this templates was just accessed
        ...
        transition: function(direction, enableBack, allowAnimate) {
          ...
          cachedAttr(enteringEle.parent(), 'nav-templates-transition', enteringData.transition);
          cachedAttr(enteringEle.parent(), 'nav-templates-direction', enteringData.direction);
          ...
          function transitionComplete() {
            ...
            // each parent nav-templates will update which views are active and cached
        ...
        cleanup: function(transData) {
          ...
          if (leavingEle && transData.direction == 'back' && !$ionicConfig.views.forwardCache()) {
            // if they just navigated back we can destroy the forward templates
  ...
  function getTransitionData(viewLocals, enteringEle, direction, view) {
    // Priority
    // 1) attribute directive on the button/link to this templates
    // 2) entering element's attribute
    // 3) entering templates's $state config property
    // 4) templates registration data
    // 5) global config
    // 6) fallback value

    var state = viewState(viewLocals);
    var viewTransition = nextTransition || cachedAttr(enteringEle, 'templates-transition') || state.viewTransition || $ionicConfig.views.transition() || 'ios';
    var navBarTransition = $ionicConfig.navBar.transition();
    direction = nextDirection || cachedAttr(enteringEle, 'templates-direction') || state.viewDirection || direction || 'none';
 ...
 *
 * Methods called directly on the $ionicListDelegate services will control all lists.
 * Use the {@link ionic.services:$ionicListDelegate#$getByHandle $getByHandle}
...
function($scope, $element, $attrs, $compile, $timeout, $ionicNavBarDelegate, $ionicConfig, $ionicHistory) {
    ...
    var headerBarInstance = {
      ...
      setItem: function(navBarItemEle, itemType) {
        ...
        if (navBarItemEle) {
          ...
          // there's a default button for this side and no templates button
  ...
  self.update = function(viewData) {
    ...
    // update the nav bar items, depending if the templates has their own or not
   ...
   * back templates. If the back button is not possible, for example, the
   * first templates in the stack, then this will not force the back button
   * to show.
   */
...
function($scope, $element, $attrs, $compile, $controller, $ionicNavBarDelegate, $ionicNavViewDelegate, $ionicHistory, $ionicViewSwitcher, $ionicConfig, $ionicScrollDelegate) {
  ...
  self.init = function() {
    ...
    // Find the details of the parent templates directive (if any) and use it
    // to derive our own qualified templates name, then hang our own details
  ...
  self.register = function(viewLocals) {
    ...
    // register that a templates is coming in and get info on how it should transition
  ...
  self.render = function(registerData, viewLocals, enteringView, leavingView, renderStart, renderEnd) {
    // register the templates and figure out where it lives in the various
    ...
    switcher.init(registerData, function() {
      // the templates is now compiled, in the dom and linked, now lets transition the views.
      // this uses a callback incase THIS nav-templates has a nested nav-templates, and after the NESTED
      // nav-templates links, the NESTED nav-templates would update which direction THIS nav-templates should use
  ...
  self.beforeEnter = function(transitionData) {
    if (isPrimary) {
      // only update this nav-templates's nav-bar if this is the primary nav-templates
   ...
   * example, the very first templates in the navigation stack would not have a
   * back templates, so the back button would be disabled.
   */
    ...
    function onRelease(ev) {
      if (isPrimary && viewTransition && dragPoints && dragPoints.length > 1) {
        ...
        if (isSwipingRight && (releaseSwipeCompletion > 0.5 || velocity > 0.1)) {
          // complete templates transition on release
          ...
          // cancel templates transition on release
...
.controller('$ionicRefresher', [
  ...
  function($scope, $attrs, $element, $ionicBind, $timeout) {
    ...
    function nativescroll(target, newScrollTop) {
      // creates a scroll event that bubbles, can be cancelled, and with its templates
...
function($scope, $attrs, $ionicSideMenuDelegate, $ionicPlatform, $ionicBody, $ionicHistory, $ionicScrollDelegate, IONIC_BACK_PRIORITY, $rootScope) {
  ...
  /**
   * Set the content templates controllers if not passed in the constructor options.
   *
...
function($scope, $element, $attrs, $compile, $rootScope) {
  ...
  var deregIonNavBarInit = $scope.$on('ionNavBar.init', function(ev, delegateHandle) {
    // this templates has its own ion-nav-bar, remember the navBarDelegateHandle for this templates
    ev.stopPropagation();
  ...
  self.init = function() {
    ...
    // add listeners for when this templates changes
  ...
  self.beforeEnter = function(ev, transData) {
    // this event was emitted, starting at intial ion-templates, then bubbles up
    // only the first ion-templates should do something with it, parent ion-views should ignore
  ...
  function generateNavBarItem(html) {
    if (html) {
      // every time a templates enters we need to recreate its templates buttons if they exist
...
function CollectionRepeatDirective($ionicCollectionManager, $parse, $window, $$rAF, $rootScope, $timeout) {
  ...
  function postLink(scope, element, attr, scrollCtrl, transclude) {
    ...
    function initAfterItemsContainer() {
      ...
      // Put everything in the templates after the repeater into a container.
      if (!container.length) {
...
function RepeatManagerFactory($rootScope, $window, $$rAF) {
  ...
  return function RepeatController(options) {
    ...
    // We say it's a grid templates if we're either dynamic or not 100% width
    ...
    // templates is a mix of list/grid methods + static/dynamic methods.
    ...
    function StaticViewType() {
      ...
      // static templates always returns the same object for getDimensions, to avoid memory allocation
        ...
        // scrolling up
          ...
          for (i = oldRenderStartIndex; i >= 0; i--) {
            if ((dim = this.getDimensions(i)) && dim.primaryPos <= scrollValue) {
              // when grid templates, make sure the render starts at the beginning of a row.
 ...
 * @name ionContent
 * @module ionic
 * @delegate ionic.services:$ionicScrollDelegate
 * @restrict E
 *
 * @description
 ...
 * directive.
 *
 * If there is any dynamic content inside the ion-content, be sure to call `.resize()` with {@link ionic.services:$ionicScrollDelegate}
 ...
 * with {@link ionic.services:$ionicScrollDelegate}.
...
.directive('ionInfiniteScroll', ['$timeout', function($timeout) {
  return {
    ...
    link: function($scope, $element, $attrs, ctrls) {
      ...
      // if this templates is not beneath a scrollCtrl, it can't be injected, proceed w/ native scrolling
      if (jsScrolling) {
...
* {@link ionic.services:$ionicListDelegate#closeOptionButtons $ionicListDelegate.closeOptionButtons}.
*
...
* @name ionList
* @module ionic
* @delegate ionic.services:$ionicListDelegate
* @codepen JsHjf
* @restrict E
* @description
...
* {@link ionic.services:$ionicListDelegate}.
 ...
 * the menu is open. Additionally, this directive will reset the entering templates's
 ...
 * app will navigate back one templates when the back button is clicked.  More advanced behavior is also
 ...
 * ```
 *
 * With custom inner markup and custom click action, using {@link ionic.services:$ionicHistory}:
 *
 * ```html
 ...
 * @name ionNavBar
 * @module ionic
 * @delegate ionic.services:$ionicNavBarDelegate
 * @restrict E
 *
 * @description
 ...
 *
 * We can add buttons depending on the currently visible templates using
 ...
 *   </ion-nav-bar>
 *
 *   <!-- where the initial templates template will be rendered -->
 *   <ion-nav-templates>
 *     <ion-templates>
 ...
 * with {@link ionic.services:$ionicNavBarDelegate}.
 ...
 * templates template the ability to specify which buttons should show in the nav bar,
 ...
 *
 * ***Note*** that `ion-nav-buttons` must be immediate descendants of the `ion-templates` or
    ...
    compile: function(tElement, tAttrs) {
      ...
      return {
        pre: function($scope, $element, $attrs, navBarCtrl) {
          ...
          if (parentViewCtrl) {
            // if the parent is an ion-templates, then these are ion-nav-buttons for JUST this ion-templates
 ...
 *
 * @description
 * The direction which the nav templates transition should animate. Available options
 ...
 * templates the ability to specify its own custom title element, such as an image or any HTML,
 * rather than being text-only. Alternatively, text-only titles can be updated using the
 * `templates-title` {@link ionic.directive:ionView} attribute.
 *
 * Note that `ion-nav-title` must be an immediate descendant of the `ion-templates` or
 * `ion-nav-bar` element (basically don't wrap it in another div).
 *
 * @usage
 * ```html
 ...
 *
 * @description
 * The transition type which the nav templates transition should use when it animates.
 ...
 * into various "states". Like Angular's core $route services, URLs can be used
 ...
 * and remember to replace ui-templates with ion-nav-templates in examples).
 *
 * @usage
 * In this example, we will create a navigation templates that contains our different states for the app.
 *
 ...
 * and then try to load home.html into the `<ion-nav-templates>`.
 *
 ...
 *   <!-- The title of the ion-templates will be shown on the navbar -->
 *   <ion-templates templates-title="Home">
 ...
 * ## Caching
 *
 * By default, views are cached to improve performance. When a templates is navigated away from, its
 * element is left in the DOM, and its scope is disconnected from the `$watch` cycle. When
 * navigating to a templates that is already cached, its scope is then reconnected, and the existing
 * element that was left in the DOM becomes the active templates. This also allows for the scroll
 ...
 * needs to know when a templates has entered or has left, then templates events emitted from the
 ...
 * If you navigate forward to the same templates again, it'll create a new DOM element and controllers
 ...
 *
 * ```html
 * <ion-templates cache-templates="false" templates-title="My Title!">
 *   ...
 * </ion-templates>
 * ```
 *
 ...
 *
 * @param {string=} name A templates name. The name should be unique amongst the other views in the
 * same state. You can have views of the same name that live in different states. For more
 * information, see ui-router's
 * [ui-templates documentation](http://angular-ui.github.io/ui-router/site/#/api/ui.router.state.directive:ui-templates).
 */
IonicModule
...
function($state, $ionicConfig) {
  ...
  return {
    ...
    compile: function(tElement, tAttrs, transclude) {
      ...
      // a nav templates element is a container for numerous views
      tElement.addClass('templates-container');
      ionic.DomUtil.cachedAttr(tElement, 'nav-templates-transition', $ionicConfig.views.transition());

      return function($scope, $element, $attr, navViewCtrl) {
        var latestLocals;

        // Put in the compiled initial templates
        ...
        function updateView(firstTime) {
          ...
          // do not update THIS nav-templates if its is not the container for the given state
          ...
          // register, update and transition to the new templates
...
function($timeout, $controller, $ionicBind) {
  return {
    ...
    controller: function() {},
    compile: function(element) {
      element.addClass('scroll-templates ionic-scroll');
 ...
 * @name ionSideMenus
 * @module ionic
 * @delegate ionic.services:$ionicSideMenuDelegate
 * @restrict E
 *
 * @description
 ...
 *
 *   <ion-side-menu-content>
 *   <!-- Main content, usually <ion-nav-templates> -->
 *   </ion-side-menu-content>
 *
 *   <!-- Right menu -->
 ...
 * with {@link ionic.services:$ionicSideMenuDelegate}.
 *
 */
 ...
 * @name ionSlideBox
 * @module ionic
 * @delegate ionic.services:$ionicSlideBoxDelegate
 * @restrict E
 * @description
 ...
 * with {@link ionic.services:$ionicSlideBoxDelegate}.
 ...
 *
 * Each ionTab has its own templates history.
 *
 * @usage
 * ```html
 * <ion-tab
 ...
 * @param {expression=} ng-click By default, the tab will be selected on click. If ngClick is set, it will not.  You can explicitly switch tabs using {@link ionic.services:$ionicTabsDelegate#select $ionicTabsDelegate.select()}.
    ...
    compile: function(element, attr) {
      ...
      if (childElementCount) {
        if (tabContentEle.children[0].tagName === 'ION-NAV-VIEW') {
          // get the name if it's a nav-templates
          navViewName = tabContentEle.children[0].getAttribute('name');
          tabContentEle.children[0].classList.add('templates-container');
          isNavView = true;
        }
      ...
      return function link($scope, $element, $attr, ctrls) {
        ...
        $scope.$on('$destroy', function() {
          if (!$scope.$tabsDestroy) {
            ...
            // is being destroyed, causing unnecessary templates loads and transitions
 ...
 * @name ionTabs
 * @module ionic
 * @delegate ionic.services:$ionicTabsDelegate
 * @restrict E
 * @codepen odqCz
 *
 * @description
 ...
 * with {@link ionic.services:$ionicTabsDelegate}.
 */
 ...
 *
 * @description
 * A container for templates content and any navigational and header bar information. When a templates
 * enters and exits its parent {@link ionic.directive:ionNavView}, the templates also emits templates
 * information, such as its title, whether the back button should be displayed or not, whether the
 * corresponding {@link ionic.directive:ionNavBar} should be displayed or not, which transition the templates
 * should use to animate, and which direction to animate.
 *
 * *Views are cached to improve performance.* When a templates is navigated away from, its element is
 * left in the DOM, and its scope is disconnected from the `$watch` cycle. When navigating to a
 * templates that is already cached, its scope is reconnected, and the existing element, which was
 * left in the DOM, becomes active again. This can be disabled, or the maximum number of cached
 * views changed in {@link ionic.provider:$ionicConfigProvider}, in the templates's `$state` configuration, or
 * as an attribute on the templates itself (see below).
 *
 * @usage
 ...
 * <ion-nav-templates>
 *   <ion-templates templates-title="My Page">
 *     <ion-content>
 *       Hello!
 *     </ion-content>
 *   </ion-templates>
 ...
 * affect your controllers logic. To know when a templates has entered or left, events
 * have been added that are emitted from the templates's scope. These events also
 * contain data about the templates, such as the title and whether the back button should
 ...
 *   <td>The templates has loaded. This event only happens once per
 * templates being created and added to the DOM. If a templates leaves but is cached,
 * then this event will not fire again on a subsequent viewing. The loaded event
 * is good place to put your setup code for the templates; however, it is not the
 * recommended event to listen to when a templates becomes active.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.enter</code></td>
 *   <td>The templates has fully entered and is now the active templates.
 * This event will fire, whether it was the first load or a cached templates.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.leave</code></td>
 *   <td>The templates has finished leaving and is no longer the
 * active templates. This event will fire, whether it is cached or destroyed.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.beforeEnter</code></td>
 *   <td>The templates is about to enter and become the active templates.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.beforeLeave</code></td>
 *   <td>The templates is about to leave and no longer be the active templates.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.afterEnter</code></td>
 *   <td>The templates has fully entered and is now the active templates.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.afterLeave</code></td>
 *   <td>The templates has finished leaving and is no longer the active templates.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.unloaded</code></td>
 *   <td>The templates's controllers has been destroyed and its element has been
 ...
 * either an individual templates basis, or by global configuration. Please see the
 * _Caching_ section in {@link ionic.directive:ionNavView} for more info.
 *
 * @param {string=} templates-title A text-only title to display on the parent {@link ionic.directive:ionNavBar}.
 * For an HTML title, such as an image, see {@link ionic.directive:ionNavTitle} instead.
 * @param {boolean=} cache-templates If this templates should be allowed to be cached or not.
 * Please see the _Caching_ section in {@link ionic.directive:ionNavView} for
 * more info. Default `true`
 * @param {boolean=} can-swipe-back If this templates should be allowed to use the swipe to go back gesture or not.
 * This does not enable the swipe to go back feature if it is not available for the platform it's running
 * from, or there isn't a previous templates. Default `true`
